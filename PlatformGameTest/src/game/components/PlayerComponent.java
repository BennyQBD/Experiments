...
public class PlayerComponent extends EntityComponent {
	...
	public PlayerComponent(Entity entity, InputListener leftKey, InputListener rightKey,
	...
	private float findLateralMovement(double moveSpeed, 
			double runModifier, double delta) {
		float moveX = 0;
	...
	public void update(double delta) {
		double oldVelY = velY;
		float moveX = getEntity().move(findLateralMovement(90.0, 2.0, delta), 0);
	...
	private void pickupCollectables() {
		...
		getEntity().visitInRange(CollectableComponent.COMPONENT_NAME,
			...
			public void visit(Entity entity, EntityComponent component) {
				val.val += ((CollectableComponent)component).getPoints();
				entity.remove();
			}
		});
		points += (int)val.val;
	...
	private void revealLocalHiddenAreas(final double range) {
		getEntity().visitInRange(SpriteComponent.COMPONENT_NAME,
			...
			@Override
			public void visit(Entity entity, EntityComponent component) {
				double distX = getEntity().getAABB().getCenterX() - entity.getAABB().getCenterX();
				double distY = getEntity().getAABB().getCenterY() - entity.getAABB().getCenterY();
				double dist = Math.sqrt(distX * distX + distY * distY)/range;

				if(entity != getEntity() && !entity.getBlocking() && entity.getDitherable()) {
				   if(dist <= 1.0) {
					   ((SpriteComponent)component).setTransparency(dist*dist);
				   } else {
					   ((SpriteComponent)component).setTransparency(1);
				   }
				}
			}
		});

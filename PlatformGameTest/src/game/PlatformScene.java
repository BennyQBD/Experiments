...
public class PlatformScene extends Scene {
	...
	private void addEntity(Config config, IInput input, 
		...
		if(color == 255) {
			...
			new SpriteComponent(entity, bitmaps.get("./res/playertest.png"));
	...
	private int getRand(int min, int range) {
		return ((int)(Math.random() * range)) % range + min;
	}

	private void addRandomBackgroundTile(ISpatialStructure<Entity> structure,
...
//			playerSprite = new ArrayBitmap("./res/playertest.png");
	...
	private Entity add(ISpatialStructure<Entity> structure,
